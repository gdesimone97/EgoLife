...
def extract_evidence(question, query_range, rag_agent, modality="video"):
    ...
    for index, id in enumerate(single_range["ids"]):
        ...
        if modality == "video":
            ...
            response = rag_agent.model.inference_video(
                video_path, int(video_start_time), start_time, end_time, video_prompt
            )
...
def get_answer(
    question, question_time, formatted_options, evidence_cards, caption_results
):
    combined_string = "\n\n".join(
