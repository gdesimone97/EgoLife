...
def gpt_inference(prompt, system_messages):
    ...
    while retry_count < max_retries:
        try:
            response = requests.post(GPT4V_ENDPOINT, headers=headers, json=payload, timeout=180)
...
def ds_inference(prompt, system_messages):
    ...
    client = ChatCompletionsClient(endpoint=endpoint, credential=AzureKeyCredential(key), transport=RequestsTransport(read_timeout=500))
        ...
        if start_time < current_window_end:
            ...
            if batch:
                ...
                try:
                    ...
                    gpt_res = ds_inference(f'All descriptions: {context}', system_message_min)
                print(gpt_res)
                if gpt_res:
                    try:
                        res = gpt_res
                        ...
                        print(f"Error extracting content from response in batch {window_index}: {e}")
                else:
...
def get_event_min(db_name):
    ...
    if os.path.exists(output_dir):
        print(f"Directory {output_dir} already exists. Checking for incomplete files...")
    else:
    ...
    json_files = [f for f in os.listdir(f'./{diary_dir}/{db_name}_l1') if f.endswith('.json')]
    merged_items = []
    ...
    with open(f'./{diary_dir}/{db_name}_l1/merged_day_1to7.json', 'w', encoding='utf-8') as f:
    ...
    for item in data:
        ...
        if not batch:
            ...
            if item_start_time < current_batch_end_time:
                ...
                gpt_res = ds_inference(f'All descriptions: {context}. Strictly output below 500 words in total.', system_message_hour)
                if gpt_res:
                    try:
                        res = gpt_res
                        results.append({
                        ...
                        print(f"Error extracting content from response in batch {batch_index}: {e}")
                    batch_index += 1
...
def get_event_hour(db_name):
    ...
    for i in range(1, 8):
        ...
        try:
            process_json_batches(input_file, f'{output_file.split(".")[0]}_tmp.json', date=i)
    ...
    json_files = [f for f in os.listdir(f'./{diary_dir}/{db_name}_l2') if f.endswith('.json')]
    merged_items = []
    for file in json_files:
    ...
    with open(f'./{diary_dir}/{db_name}_l2/merged_day_1to7.json', 'w', encoding='utf-8') as f:
        json.dump(merged_items, f, ensure_ascii=False, indent=4)
...
def get_event_day(db_name):
    ...
    for day in range(1, 8):
        ...
        try:
            ...
            gpt_res = gpt_inference(f'All descriptions: {context}. The output should be around 1500 words in total.', system_message_day)
            if gpt_res:
                try:
                    res = gpt_res
                    day_summary = {
            ...
            with open('error_file.json', 'w') as f:
                json.dump([day, day_data[0]['start_time'], day_data[-1]['end_time']], f, indent=4)


    with open(f'./{diary_dir}/{db_name}_l3/merged_day_1to7.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Process events with specified database name and diary directory')
    parser.add_argument('--db_name', type=str, help='Name of the database to process')
    parser.add_argument('--diary_dir', type=str, default='events_diary', help='Directory of the event diary (default: events_diary)')
