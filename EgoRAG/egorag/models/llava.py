...
class LLaVA(BaseQueryModel):
    def __init__(self, pretrained):
        ...
        self.conv_template = "vicuna_v1" if 'next' in self.model_name.lower() else 'qwen_1_5'
        print(self.conv_template)
        self.tokenizer, self.model, self.image_processor, self.max_length = load_pretrained_model(pretrained, None,self.model_name, torch_dtype="bfloat16", device_map=self.device_map)  
        self.model.eval()
    ...
    def inference_video(self, video_path, video_start_time, start_time, end_time, human_query, system_message=None):
        ...
        frames = self.image_processor.preprocess(processed_data, return_tensors="pt")["pixel_values"].cuda().bfloat16()
        video = [frames]
        ...
        attention_masks = input_ids.ne(self.tokenizer.pad_token_id).long().cuda().bfloat16()
